import { BarChart3, Brain, CalendarDays, Columns3, CheckSquare, Download, LogOut, Moon, RotateCcw, Search, Settings, Sun, Timer } from "lucide-react";
import { useCallback, useEffect, useState } from "react";
import { AppData } from "../types";
import { SyncStatus } from "../hooks/usePersistentApp";
import { initNativeBridge, setupBackButton, setStatusBarStyle, scheduleReviewReminder } from "../lib/notifications";
...
export function AppShell({
  view, setView, theme, toggleTheme, children, userName, syncStatus, onLogout, data,
}: {
  view: ViewKey; setView: (view: ViewKey) => void; theme: "light" | "dark"; toggleTheme: () => void;
  children: React.ReactNode; userName: string; syncStatus: SyncStatus; onLogout: () => void;
  data?: AppData;
}) {
  ...
  // 初始化原生桥接 + 设置每日复盘提醒
  useEffect(() => {
    initNativeBridge().then(() => {
      scheduleReviewReminder();
    });
  }, []);

  // Android 返回键：回到首页而非退出
  useEffect(() => {
    setupBackButton(() => setView("dashboard"));
  }, [setView]);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-ink-50 pb-44 text-ink-900 transition dark:bg-[#10120f] dark:text-ink-50 lg:pb-28">
      ...
      <header className="sticky top-0 z-20 border-b border-white/70 bg-ink-50/78 backdrop-blur-xl dark:border-white/10 dark:bg-[#10120f]/82">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:py-4">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Button variant="secondary" onClick={() => setSearchOpen(true)} title="搜索 (Ctrl+K)" className="h-10 w-10 px-0 hidden sm:inline-flex">
              <Search size={16} />
            </Button>
      ...
      {data && <SearchModal data={data} open={searchOpen} onClose={() => setSearchOpen(false)} onNavigate={(v) => { setView(v as ViewKey); setSearchOpen(false); }} />}
    </div>
  );
}
