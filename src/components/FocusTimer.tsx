...
// 浏览器桌面通知
function notifyBrowser(minutes: number) {
  if (!("Notification" in window)) return;
  if (Notification.permission === "granted") {
    new Notification("计时结束", { body: `${minutes} 分钟专注完成，来做个复盘吧。`, icon: "/icon.svg" });
      ...
      setTimer((current) => {
        ...
        if (remaining <= 0) {
          ...
          if (reflectedTimerRef.current !== reflectionKey) {
            ...
            scheduleTimerEndNotification(current.modeMinutes); notifyBrowser(current.modeMinutes);
          }
        ...
        <Panel>
          ...
          <div className="mt-4 grid gap-3">
            {data.timerReflections.slice(0, 5).map((item) => {
              const task = item.taskId ? data.tasks.find((t) => t.id === item.taskId) : null;
              return (
                <div key={item.id} className="flex items-start justify-between rounded-md border border-ink-200 p-3 text-sm dark:border-white/10">
                  ...
                  <div className="shrink-0 ml-3 text-right text-xs text-ink-700/45 dark:text-ink-100/35">
                    {task && <div className="truncate max-w-[120px]">{task.title}</div>}
                    <div>{item.modeMinutes} 分钟</div>
                  </div>
                </div>
              );
            })}
